...
# 日期数据转换  f-str ->2019.01.30 15:29
def timef2str(dt):
    print(dt)
    b = time.gmtime(dt)
    print(b)
    str_c = time.strftime('%Y.%m.%d %H:%M', b)
    return str_c
...
class excel_read:
    def __init__(self, excel_path=r'E:\project\test\suiinput\twomenu.xls', encoding='utf-8', index=0):
...
# 分类
    def input_levelSelect(self, id="levelSelect-payout", menu1="ls-li-payout-2032594114994", menu2=" ls-li-payout-2032594114979"):
...
if __name__ == '__main__':
    ...
    # 导入控制流（）
    ctrls = excel_read(
        excel_path=r'E:\project\test\suiinput\ctrls.xls').get_data()
    # print(ctrls)
    ...
    # 导入datas
    datas = excel_read(
        excel_path=r'E:\project\test\suiinput\datas.xls').get_data()

    # 创建日志文件
    t_str = time.strftime("%Y%m%d%H%M", time.localtime())
    filename = 'E:\\project\\test\\suioutput\\' + t_str + '.log'
    ...
    for data in datas:
        ...
        chrome.input_levelSelect(
            id=ctrls[2][1], menu1=item1[1], menu2=item2[1])  # 分类
